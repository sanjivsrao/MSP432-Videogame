...
/**
 * Receives the ASCII integer in the data packet using the UART_receiveData function.
 * The integer is automatically converted into a character and returned by the function.
 * @param uart_p: The pointer to the UART instance with which to handle our operations.
 *
 * @return the ASCII character of the data in the packet
 */
char UART_receiveChar(UART* uart_p)
{
    char character = UART_receiveData(uart_p->moduleInstance);
    return character;
}

/**
 * Determines if the user has sent a UART data packet to the board by checking
 * the transmit interrupt status of the proper system UART module.
 *
 * @param uart_p: The pointer to the UART instance with which to handle our operations.
 *
 * @return true if the UART is ready to transmit data, and false otherwise
 */
...
/**
 * Converts the character the user intends into an integer and transmits the data packet
 * to the board.
 * @param uart_p: The pointer to the UART instance with which to handle our operations.
 *
 * @param c: The character being transmitted
 * Function does not have a return value.
 */
void UART_sendChar(UART* uart_p, char c)
{
    uint_fast8_t c_int = c;
    UART_transmitData(uart_p->moduleInstance, c_int);
}
