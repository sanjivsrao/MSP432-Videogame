...
void Init_LCD();
void InitGraphics(Graphics_Context *gfxContext_p);
/**
...
HAL HAL_construct()
{
    ...
    /// Construct and initialize the graphics context
    hal.gfxContext = GFX_construct(GRAPHICS_COLOR_WHITE, GRAPHICS_COLOR_BLACK);
...
void InitGraphics(Graphics_Context *gfxContext_p) {
    ...
    Graphics_initContext(gfxContext_p,
                         &g_sCrystalfontz128x128,
                         &g_sCrystalfontz128x128_funcs);
    Graphics_setForegroundColor(gfxContext_p, GRAPHICS_COLOR_YELLOW);
    Graphics_setBackgroundColor(gfxContext_p, GRAPHICS_COLOR_BLACK);
    Graphics_setFont(gfxContext_p, &g_sFontCmtt16);

    Init_LCD();

    Graphics_clearDisplay(gfxContext_p);
}
